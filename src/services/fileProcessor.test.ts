...
describe('processDroppedItems', () => {
  it('handles dropped entries even when no abort signal is available yet', async () => {
    ...
    await expect(processDroppedItems(items, vi.fn(), undefined as unknown as AbortSignal)).resolves.toMatchObject({
...
describe('processFiles', () => {
  it('does not load the zip processor for regular files', async () => {
    ...
    await processFiles([file], vi.fn(), true, 0, new AbortController().signal);
  ...
  it('skips files that exceed the byte-size threshold without reading content', async () => {
    ...
    const result = await processFiles([file], vi.fn(), true, 1_000, new AbortController().signal);
  ...
  it('loads the gitignore matcher module when ignore files are present', async () => {
    ...
    await processFiles([gitignore, keptFile], vi.fn(), true, 100_000, new AbortController().signal);
  ...
  it('loads the glob matcher module when include patterns are configured', async () => {
    ...
    await processFiles([keptFile], vi.fn(), true, 100_000, new AbortController().signal, {
  ...
  it('preserves empty directories discovered inside zip archives', async () => {
    ...
    const result = await processFiles([zipFile], vi.fn(), true, 100_000, new AbortController().signal, {
  ...
  it('throws a descriptive error when zip extraction fails', async () => {
    ...
    await expect(processFiles([zipFile], vi.fn(), true, 100_000, new AbortController().signal)).rejects.toThrow(
      'Failed to unzip broken.zip: corrupt archive'
    );
  });
  ...
  it('treats a zero maxCharsThreshold as disabled and still extracts content', async () => {
    ...
    const result = await processFiles([file], vi.fn(), true, 0, new AbortController().signal);
  ...
  it('skips macOS metadata files like .DS_Store from exported file contents', async () => {
    ...
    const result = await processFiles([dsStore], vi.fn(), true, 0, new AbortController().signal);
