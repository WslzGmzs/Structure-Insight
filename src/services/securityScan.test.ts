...
describe('scanSensitiveContent', () => {
  it('detects openai-style keys and private keys', () => {
    const findings = scanSensitiveContent(
      'src/.env',
      ['OPENAI_API_KEY="sk-proj-abcdefghijklmnopqrstuvwxyz123456"', '-----BEGIN PRIVATE KEY-----'].join('\n')
    );

    expect(findings.map((finding) => finding.ruleId)).toEqual(['openai-api-key', 'private-key']);
  });
