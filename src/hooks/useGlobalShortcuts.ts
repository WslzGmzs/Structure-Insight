...
export function useGlobalShortcuts({
    ...
    React.useEffect(() => {
        const handleGlobalKeys = (event: KeyboardEvent) => {
            ...
            if ((event.ctrlKey || event.metaKey) && isHandledShortcutKey(normalizedKey) && !isEditableEventTarget(event.target)) {
            ...
            if (event.key === 'Escape') {
                if (isShortcutsOpen) { event.preventDefault(); setIsShortcutsOpen(false); }
                else if (isSearchOpen) { event.preventDefault(); setIsSearchOpen(false); }
                else if (isFileRankOpen) { event.preventDefault(); setIsFileRankOpen(false); }
                else if (isSecurityFindingsOpen) { event.preventDefault(); setIsSecurityFindingsOpen(false); }
                else if (isSettingsOpen) { event.preventDefault(); setIsSettingsOpen(false); }
                else if (isLoading) { event.preventDefault(); handleCancel(); }
            }
        };
