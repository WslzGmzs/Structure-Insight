...
import { parsePatternList } from '../services/patternList';
import {
  deleteRecentProjectHandle,
  loadRecentProjectHandle,
  saveRecentProjectHandle,
} from '../services/recentProjectHandles';
import type { ProcessedFiles, RecentProject } from '../types';
...
export const useFileProcessing = ({
  ...
  const applyRecentProjectRecord = React.useCallback(
    async (project: RecentProject, directoryHandle?: FileSystemDirectoryHandle) => {
  ...
  const resolveExistingProjectId = React.useCallback(
  ...
  const handleFileSelect = async () => {
    if (window.showDirectoryPicker) {
      try {
        ...
        if (!isErrorNamed(error, 'AbortError') && !isErrorNamed(error, 'NotAllowedError')) {
    ...
    input.onchange = (event) => {
      ...
      if (target instanceof HTMLInputElement && target.files) {
        ...
        handleProcessing(Array.from(target.files));
      }
    };
    input.click();
  };
  ...
  const handleRecentProjectSelect = async (project: RecentProject) => {
    try {
      ...
      const permission =
        queryPermission === 'granted' ? queryPermission : await directoryHandle.requestPermission({ mode: 'read' });
