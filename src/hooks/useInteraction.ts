...
import type { ConfirmationState, FileNode, ProcessedFiles } from '../types';
...
export const useInteraction = ({
  ...
  const handleToggleExclude = (path: string) => {
    setProcessedData((prevData) => {
      ...
      // Update FileContents
      const newFileContents = prevData.fileContents.map((f) => (f.path === path ? { ...f, excluded: !f.excluded } : f));
