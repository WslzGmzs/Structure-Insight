    ...
    <div className="max-w-3xl mx-auto w-full space-y-6">
        <SectionGroup title="导出结构" icon="fa-layer-group">
            <SettingsRow label="导出格式" description="决定上下文输出的最终载体。">
                <div className="w-full sm:w-64">
                    <select
                        ...
                        onChange={(event) => settings.onSetExportFormat(event.target.value as ExportFormat)}
            ...
            <SettingsRow
                ...
                control={<Toggle id="file-summary-toggle" label="文件摘要" checked={settings.includeFileSummary} onChange={settings.onToggleIncludeFileSummary} />}
            />
            <SettingsRow
                label="目录结构"
                description="保留完整目录树，方便快速理解仓库轮廓。"
                control={<Toggle id="directory-structure-toggle" label="目录结构" checked={settings.includeDirectoryStructure} onChange={settings.onToggleIncludeDirectoryStructure} />}
            />
        </SectionGroup>
        ...
        <SectionGroup title="内容处理" icon="fa-wand-magic-sparkles">
            <SettingsRow
                ...
                control={<Toggle id="line-numbers-toggle" label="导出时显示行号" checked={settings.showLineNumbers} onChange={settings.onToggleShowLineNumbers} />}
            />
            <SettingsRow
                label="移除空行"
                description="收紧输出内容的空白体积。"
                control={<Toggle id="remove-empty-lines-toggle" label="移除空行" checked={settings.removeEmptyLines} onChange={settings.onToggleRemoveEmptyLines} />}
            />
            <SettingsRow
                label="截断 Base64"
                description="避免长 data URL 直接淹没上下文。"
                control={<Toggle id="truncate-base64-toggle" label="截断 Base64" checked={settings.truncateBase64} onChange={settings.onToggleTruncateBase64} />}
            />
            <SettingsRow label="拆分阈值" description="大于 0 时按字符数自动拆分导出文件。">
                <div className="w-full sm:w-48">
                    <Field
                        ...
                        type="number"
                        min={0}
                        value={settings.exportSplitMaxChars}
                        onChange={(value) => settings.onSetExportSplitMaxChars(Math.max(0, Number(value) || 0))}
                    />
                </div>
            </SettingsRow>
        </SectionGroup>
        ...
        <SectionGroup title="过滤规则" icon="fa-filter">
            <SettingsRow label="包含模式" description="例如 `src/**/*.ts`、`docs/**`。" stacked>
                <Field
                    ...
                    onChange={settings.onSetIncludePatterns}
            ...
            <SettingsRow label="忽略模式" description="例如 `**/*.test.ts`、`dist/**`。" stacked>
                <Field
                    ...
                    onChange={settings.onSetIgnorePatterns}
            ...
            <SettingsRow
                ...
                control={<Toggle id="default-patterns-toggle" label="默认忽略规则" checked={settings.useDefaultPatterns} onChange={settings.onToggleUseDefaultPatterns} />}
            />
            <SettingsRow
                label="应用忽略文件"
                description="复用项目里的 `.gitignore`、`.ignore` 与 `.repomixignore` 规则。"
                control={<Toggle id="gitignore-toggle" label="应用忽略文件" checked={settings.useGitignore} onChange={settings.onToggleUseGitignore} />}
            />
            <SettingsRow
                label="包含空目录"
                description="导出目录树时保留空目录节点。"
                control={<Toggle id="empty-directories-toggle" label="包含空目录" checked={settings.includeEmptyDirectories} onChange={settings.onToggleIncludeEmptyDirectories} />}
            />
        </SectionGroup>
        ...
        <SectionGroup title="附加说明" icon="fa-pen-ruler">
            <SettingsRow label="Header 文本" description="适合放项目背景、任务上下文或外部限制。" stacked>
                <Field
                    ...
                    onChange={settings.onSetExportHeaderText}
            ...
            <SettingsRow label="Instruction 文本" description="适合告诉 AI 希望得到的输出形式。" stacked>
                <Field
                    ...
                    multiline
                    rows={3}
                    value={settings.exportInstructionText}
                    onChange={settings.onSetExportInstructionText}
