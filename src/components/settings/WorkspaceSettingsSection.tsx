...
const ThemeSegmentedControl = ({ isDarkTheme, onToggleTheme }: { isDarkTheme: boolean; onToggleTheme: () => void }) => (
...
export const WorkspaceSettingsSection = ({ settings }: WorkspaceSettingsSectionProps) => {
  ...
  const maxCharsThresholdInKb = Math.round(values.maxCharsThreshold / 1024);

  return (
    <div className="max-w-3xl mx-auto w-full space-y-6">
      <SectionGroup title="外观" icon="fa-palette">
        <SettingsRow label="主题模式" description="在浅色与深色阅读环境之间快速切换。">
          <ThemeSegmentedControl isDarkTheme={values.isDarkTheme} onToggleTheme={actions.onToggleTheme} />
        </SettingsRow>
        <SettingsRow label="字体大小" description="即时调整阅读密度。" stacked>
          <div className="space-y-3">
            ...
            <input
              ...
              onChange={(event) => actions.onSetFontSize(Number(event.target.value))}
        ...
        <SettingsRow
          ...
          control={
            <Toggle
              ...
              onChange={actions.onToggleWordWrap}
            />
          }
        />
        <SettingsRow
          ...
          control={
            <Toggle
              id="extract-toggle"
              label="提取文件内容"
              checked={values.extractContent}
              onChange={actions.onToggleExtractContent}
            />
          }
        />
          ...
          <div className="flex items-center gap-2">
            <div className="w-24">
              <Field
                ...
                onChange={(value) => actions.onSetMaxCharsThreshold(Math.max(0, Number(value) || 0) * 1024)}
              />
            </div>
            <span className="text-xs font-semibold uppercase tracking-[0.16em] text-light-subtle-text dark:text-dark-subtle-text">
              KB
      ...
      <section className="rounded-2xl bg-gradient-to-br from-red-600 to-red-700 p-5 text-white shadow-lg shadow-red-900/20">
        <div className="mb-4 flex items-center gap-2 border-b border-white/10 pb-2">
          ...
          <h4 className="text-xs font-bold uppercase tracking-[0.18em] text-white">危险操作</h4>
        </div>
