...
const Header: React.FC<HeaderProps> = ({
  ...
  const secondaryButtonClass =
    'inline-flex h-8 shrink-0 items-center justify-center gap-1.5 rounded-lg px-2 text-xs font-medium text-light-subtle-text transition-colors hover:bg-light-hover hover:text-light-text disabled:cursor-not-allowed disabled:opacity-50 dark:text-dark-subtle-text dark:hover:bg-dark-hover dark:hover:text-dark-text';
  const primaryButtonClass =
    'inline-flex h-8 shrink-0 items-center justify-center gap-1.5 rounded-lg bg-primary px-2.5 text-xs font-semibold text-white shadow-sm shadow-primary/20 transition-colors hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-60 sm:px-3';
  ...
  return (
    <header
      className="flex h-12 shrink-0 items-center justify-between gap-2 overflow-hidden border-b border-light-border bg-light-header px-2 py-1 shadow-sm shadow-slate-900/5 dark:border-dark-border dark:bg-dark-header dark:shadow-black/20 sm:px-3 z-20"
      role="banner"
    >
      <div className="flex min-w-0 shrink-0 items-center gap-2">
        <a
          href="https://structure-insight-website.pages.dev/"
          target="_blank"
          rel="noopener noreferrer"
          title="访问 Structure Insight 主页"
        >
          <img src={iconUrl} alt="Structure Insight Logo" className="h-8 w-8" />
        </a>
        <div className="hidden min-w-0 lg:block">
          <h1 className="truncate text-sm font-bold tracking-tight text-light-text dark:text-dark-text">
            Structure Insight
          </h1>
          <p className="text-[9px] font-medium uppercase tracking-[0.16em] text-light-subtle-text dark:text-dark-subtle-text">
            本地仓库上下文
          </p>
        </div>
      </div>
      ...
      <div
        className="no-scrollbar flex min-w-0 flex-1 flex-nowrap items-center justify-end gap-1 overflow-x-auto"
        role="toolbar"
        aria-label="工具栏"
      >
        <button
        ...
        {hasContent && (
          <>
            <div
              ...
              className="flex shrink-0 items-center gap-0.5 rounded-xl border border-light-border/70 bg-light-panel/70 p-0.5 dark:border-dark-border/70 dark:bg-dark-panel/70"
            >
              <button
                onClick={onShowCode}
        ...
        {isLoading ? (
          <button
            onClick={onCancel}
            className={`${secondaryButtonClass} !text-red-500 hover:!bg-red-500/10`}
            title="取消 (Esc)"
            aria-label="取消"
          >
