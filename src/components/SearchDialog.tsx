...
function getViewportSafePosition(x: number, y: number, width: number, height: number) {
...
const SearchDialog: React.FC<SearchDialogProps> = ({ onClose, onSearch, onNavigate, resultsCount, currentIndex }) => {
  ...
  const [position, setPosition] = React.useState(() =>
    clampPosition(window.innerWidth - DIALOG_MAX_WIDTH - DIALOG_MARGIN, 70)
  );
  ...
  const handleMouseMove = React.useCallback(
    (e: MouseEvent) => {
      ...
      setPosition(clampPosition(e.clientX - dragStartPos.current.x, e.clientY - dragStartPos.current.y));
    },
  ...
  const runSearch = React.useCallback(
    (newQuery: string, newOptions: SearchOptions) => {
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          aria-label="关闭搜索"
          className="w-7 h-7 rounded-full hover:bg-light-border dark:hover:bg-dark-border flex items-center justify-center"
        >
        ...
        <form onSubmit={handleSearch}>
          <div className="flex space-x-2">
                  ...
                  <ul>
                    {history.map((h, i) => (
                      <li
                        ...
                        className="px-3 py-2 text-sm cursor-pointer hover:bg-light-border dark:hover:bg-dark-border history-item"
                      >
                        {h}
                      </li>
                    ))}
            ...
            <button
              ...
              className="w-10 h-10 rounded-md bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border disabled:opacity-50 flex items-center justify-center hover:bg-light-border dark:hover:bg-dark-border/50"
            >
