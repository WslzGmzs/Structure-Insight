      ...
      <div className="relative z-10 mx-auto flex min-h-full w-full max-w-5xl flex-col items-center justify-start px-4 py-10 pb-16 sm:px-6 lg:justify-center">
        ...
        <div className="text-center mb-10 px-2">
          ...
          <h1 className="mx-auto max-w-[10ch] text-3xl font-extrabold leading-tight tracking-tight text-light-text dark:text-dark-text sm:max-w-none sm:text-4xl md:text-6xl mb-5">
            Structure{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-cyan-600 to-amber-500 dark:from-primary-disabled dark:via-cyan-300 dark:to-amber-300">
              Insight
            </span>
          </h1>
          ...
          >
            ...
            <div className="relative flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-lg sm:text-xl font-bold text-light-text dark:text-dark-text mb-1 group-hover:text-primary transition-colors">
                  选择项目文件夹
                </h3>
                <p className="text-xs sm:text-sm text-light-subtle-text dark:text-dark-subtle-text">
                  支持文件夹、ZIP 与拖放导入，几秒内生成可导航结构。
                </p>
              </div>
        ...
        <div className="mb-10 grid w-full max-w-4xl grid-cols-1 gap-3 sm:grid-cols-3">
          {capabilities.map((item) => (
            <div
              key={item.title}
              className="rounded-2xl border border-light-border bg-light-panel/80 p-4 shadow-sm dark:border-dark-border dark:bg-dark-panel/80"
            >
              ...
              <p className="mt-1 text-xs leading-5 text-light-subtle-text dark:text-dark-subtle-text">
                {item.description}
              </p>
            </div>
          ))}
        ...
        {recentProjects.length > 0 && (
          <div className="w-full max-w-lg mb-10">
            <p className="text-xs font-medium text-light-subtle-text dark:text-dark-subtle-text mb-3 uppercase tracking-wider">
              最近项目
            </p>
