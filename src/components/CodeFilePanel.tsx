...
export interface CodeFilePanelProps {
  file: FileContent;
  isEditing: boolean;
...
const IconButton: React.FC<{ icon: string; title: string; onClick: () => void; disabled?: boolean; text?: string }> = ({
  icon,
  title,
  onClick,
  disabled,
  ...
  <button
    onClick={onClick}
    className="flex items-center space-x-1.5 text-sm text-light-subtle-text dark:text-dark-subtle-text hover:text-light-text dark:hover:text-dark-text disabled:opacity-50 transition-colors"
    title={title}
    disabled={disabled}
  >
    ...
    <div className="flex flex-col gap-3 p-3 bg-light-header/80 dark:bg-dark-header/80 border-b border-light-border dark:border-dark-border sticky top-0 z-[1] sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0 flex items-center gap-3" title={file.path}>
        ...
        <div className="min-w-0">
          <div
            className={`truncate font-mono text-sm font-semibold text-light-text dark:text-dark-text ${
              file.excluded ? 'line-through text-light-subtle-text dark:text-dark-subtle-text italic' : ''
            }`}
          >
            {fileName} {file.excluded && '(已排除)'}
          </div>
          <div
            data-file-path-display="full"
            className="mt-0.5 flex min-w-0 items-center gap-1.5 text-xs text-light-subtle-text dark:text-dark-subtle-text"
          >
        ...
        <button
          ...
          className="text-light-subtle-text hover:text-primary transition-colors flex items-center justify-center p-1.5 rounded-lg hover:bg-light-border dark:hover:bg-dark-border/50"
          title="复制完整路径"
          aria-label="复制完整路径"
        >
      ...
      <div className="flex items-center gap-2 text-xs text-light-subtle-text dark:text-dark-subtle-text shrink-0 sm:ml-2">
        <span className="hidden rounded-full border border-light-border px-2 py-1 dark:border-dark-border sm:inline">
          {file.stats.lines} 行
        </span>
        <span className="hidden rounded-full border border-light-border px-2 py-1 dark:border-dark-border sm:inline">
          {file.stats.chars} 字符
        ...
        {isMarkdown && !file.excluded && (
          <IconButton
            onClick={() => onToggleMarkdownPreview(file.path)}
            title={isMarkdownPreview ? '显示原文' : '预览 Markdown'}
            disabled={isEditing}
            icon={isMarkdownPreview ? 'fa-file-code' : 'fa-eye'}
          />
        )}
        {!file.excluded && (
          <>
            <IconButton
              onClick={() => onStartEdit(file.path)}
              title="编辑内容"
              disabled={isEditing}
              icon="fa-pen-to-square"
            />
            <IconButton
              onClick={() => onCopyContent(file.content)}
              title="复制内容"
              disabled={isEditing}
              icon="fa-copy"
            />
...
const CodeFilePanel = ({
  ...
  const codeRef = useCodeHighlighting({
    activeMatchIndexInFile,
    file,
    isEditing,
    isMarkdownPreview,
  ...
  return (
    <div
      className={`bg-light-panel dark:bg-dark-panel rounded-lg overflow-hidden border border-light-border dark:border-dark-border focus-within:ring-2 focus-within:ring-primary ${file.excluded ? 'opacity-75' : ''}`}
    >
      <FileContentHeader
        file={file}
        isEditing={isEditing}
      ...
      {file.excluded ? (
        ...
        <div className="p-4">
          ...
          <div className="flex justify-end space-x-2 mt-2">
            <button
              ...
              className="px-3 py-1 rounded-md text-sm bg-gray-200 dark:bg-dark-border hover:bg-gray-300 dark:hover:bg-gray-600"
            >
              取消
            </button>
            <button
              onClick={() => onSaveEdit(file.path, editText)}
              className="px-3 py-1 rounded-md text-sm bg-primary text-white hover:bg-primary-hover"
            >
              保存
            </button>
          </div>
        ...
        <div className="flex" style={codeStyle}>
          ...
          <div className="relative flex-1">
            <pre
              className={`py-3 pr-3 bg-light-bg dark:bg-dark-bg ${
                wordWrap ? 'whitespace-pre-wrap break-words' : 'whitespace-pre overflow-x-auto'
              }`}
            >
              <code ref={codeRef} className={`language-${file.language} hljs`} />
            </pre>
          </div>
        </div>
      )}
...
export default React.memo(CodeFilePanel);
