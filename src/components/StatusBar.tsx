...
const StatusBarItem: React.FC<{ icon: string; value: number | string; label: string }> = ({ icon, value, label }) => (
  <span
    className="inline-flex h-7 shrink-0 items-center gap-1.5 rounded-full border border-light-border bg-light-panel px-2.5 text-light-subtle-text dark:border-dark-border dark:bg-dark-panel dark:text-dark-subtle-text"
    title={label}
  >
...
const StatusBar: React.FC<StatusBarProps> = ({
  ...
  const typeSummary = React.useMemo(() => {
    ...
    const sorted = [...counts.entries()].sort((a, b) => b[1] - a[1] || compareFileTypeLabels(a[0], b[0])).slice(0, 3);
  ...
  return (
    <footer className="min-h-10 flex items-center gap-2 overflow-x-auto whitespace-nowrap px-3 py-1.5 bg-light-header dark:bg-dark-header border-t border-light-border dark:border-dark-border text-xs text-light-subtle-text dark:text-dark-subtle-text shrink-0">
      {selectedFileName && (
        <span
          className="inline-flex h-7 items-center gap-1.5 shrink-0 mr-auto min-w-0 rounded-full border border-light-border bg-light-panel px-2.5 dark:border-dark-border dark:bg-dark-panel"
          title="当前文件"
        >
          ...
          <span className="truncate max-w-[40vw] font-medium text-light-text dark:text-dark-text sm:max-w-none">
            {selectedFileName}
          </span>
        </span>
      )}
      ...
      {typeSummary && (
        <span
          className="inline-flex h-7 items-center gap-1.5 shrink-0 rounded-full border border-light-border bg-light-panel px-2.5 dark:border-dark-border dark:bg-dark-panel"
          title="文件类型分布"
        >
          ...
          <span className="truncate max-w-[35vw] font-medium text-light-text dark:text-dark-text sm:max-w-none">
            {typeSummary}
          </span>
        </span>
      )}
