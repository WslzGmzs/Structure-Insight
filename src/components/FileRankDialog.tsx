...
const FileRankDialog: React.FC<FileRankDialogProps> = ({
  isOpen,
  onClose,
  files,
  onSelectFile,
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {sortedFiles.length === 0 ? (
            <div className="text-center p-8 text-light-subtle-text dark:text-dark-subtle-text">没有可显示的文件</div>
          ) : (
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex items-center w-full gap-4">
                      <span
                        className={`font-mono text-sm w-8 text-center shrink-0 ${index < 3 ? 'text-primary font-bold' : 'text-light-subtle-text dark:text-dark-subtle-text'}`}
                      >
                        #{index + 1}
                      </span>
                      ...
                      <div className="flex-1 min-w-0">
                        <div
                          className={`text-sm font-medium text-light-text dark:text-dark-text truncate ${isExcluded ? 'line-through' : ''}`}
                          title={file.path}
                        >
                          {file.path}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        <div className="text-sm font-bold text-primary tabular-nums">
                          {file.stats.chars.toLocaleString()}{' '}
                          <span className="text-xs font-normal opacity-70">字符</span>
                        </div>
