...
describe('SearchDialog', () => {
  it('adds extra right padding when showing result counts so typed text stays visible', () => {
    render(
      <SearchDialog onClose={vi.fn()} onSearch={vi.fn()} onNavigate={vi.fn()} resultsCount={12} currentIndex={1} />
    );
  ...
  it('uses viewport-safe sizing on narrow screens and labels dialog controls', () => {
    const { container } = render(
      <SearchDialog onClose={vi.fn()} onSearch={vi.fn()} onNavigate={vi.fn()} resultsCount={0} currentIndex={null} />
    );
